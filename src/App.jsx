...
function normalizarDB(db) {
  ...
  if (db.planilhaWebhookUrl == null) db.planilhaWebhookUrl = "";
  return db;
}
...
// As gravações na nuvem são encadeadas em fila: sem a fila, dois saves rápidos podiam
...
async function saveDB(db) {
  ...
  let nuvemOk = null;
  if (nuvemConfigurada()) {
    filaNuvem = filaNuvem.then(() => nuvemSalvar(db)).then(() => true, () => false);
    nuvemOk = await filaNuvem;
  }
...
/* Modo "Planilha de Comissões": aba com as colunas
   NOME | POSITIVO (ou MOVIMENTO) | PERCENTUAL | VALOR EFETIVO | RECEBER | PAGAMENTO
...
function montarDeComissoes(bruto, db) {
  ...
  const idxHeader = bruto.findIndex((r) => {
    const up = r.map((c) => String(c).trim().toUpperCase());
    return up.some((c) => c.endsWith("NOME")) && up.some((c) => c.startsWith("POSITIVO") || c.startsWith("MOVIMENTO"));
  });
  ...
  const iNome = header.findIndex((h) => h.endsWith("NOME"));
  let iPos = col("POSITIVO"); if (iPos === -1) iPos = col("MOVIMENTO");
...
async function sincronizarPlanilha(url, db, update) {
  ...
  if (!cRows?.length || !("Nome" in cRows[0]) || !lRows?.length || !("Cambista" in lRows[0])) {
    ...
    if (!montado) throw new Error('A planilha está vazia ou fora do modelo. Use a aba COMISSÕES (colunas NOME, MOVIMENTO ou POSITIVO, PERCENTUAL, PAGAMENTO) ou as abas "Cambistas"/"Lancamentos" do arquivo exportado.');
  ...
  // Aba GASTOS (opcional): antes só era lida no modo Comissões — o modo completo a ignorava
  ...
  update((d) => {
    ...
    if (gastosCompleto !== null) d.gastos = gastosCompleto;
    d.planilhaUrl = url;
    d.planilhaUltimaSync = agora;
  });
...
/* ---- Escrita site → planilha (Web App do Google Apps Script) ----
   ...
   precisa de um Web App do Apps Script (código em apps-script-planilha.gs, na raiz
   do projeto) que recebe este POST e reescreve as abas Cambistas, Lancamentos,
   Pagamentos e Gastos. O POST vai sem cabeçalhos extras para não exigir preflight
   de CORS (o Apps Script não responde a OPTIONS). */
async function planilhaEnviar(webhookUrl, db) {
  ...
  const payload = {
    ...
    lancamentos: [...db.lancamentos].sort((a, b) => a.data.localeCompare(b.data)).map((l) => ({
      ...
      positivo: l.positivo,
      percentual: l.pct != null ? l.pct : (cambById[l.cambistaId]?.comissaoPadrao || 0),
    })),
    pagamentos: (db.pagamentos || []).map((p) => ({ data: fmtData(p.data), cambista: cambById[p.cambistaId]?.nome || "", valor: p.valor, obs: p.obs || "" })),
    gastos: (db.gastos || []).map((g) => ({ data: fmtData(g.data), categoria: g.categoria || "", responsavel: g.responsavel || "", descricao: g.descricao || "", valor: g.valor })),
  };
  ...
  // Web App sem acesso "Qualquer pessoa" devolve a tela de login do Google com
  // status 200 — só o JSON {ok:true} do script confirma que a escrita aconteceu
  const texto = await res.text();
  let corpo; try { corpo = JSON.parse(texto); } catch { corpo = null; }
  if (!corpo || corpo.ok !== true) throw new Error("O Web App não confirmou a escrita (verifique o acesso \"Qualquer pessoa\").");
}
...
export default function App() {
  ...
  const [planilhaErro, setPlanilhaErro] = useState(false);
  const [showInativeModal, setShowInativeModal] = useState(false);
  const [inativos, setInativos] = useState([]);
  const primeira = useRef(true);
  const timerPlanilha = useRef(null);
        ...
        <div className="bg-slate-900 border-b-2 border-orange-500">
          <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 flex items-center h-14">
            ...
            <div className="ml-auto lg:ml-0 flex items-center">
              <span className="flex items-center text-[11px] text-slate-500 mr-2 sm:mr-4" title={nuvemErro ? "Os dados estão salvos neste navegador, mas a nuvem não respondeu" : planilhaErro ? "Salvo na nuvem, mas a planilha do Google não recebeu a última mudança" : undefined}>
                <Circle size={7} className={`sm:mr-1.5 ${salvando ? "text-amber-400 fill-amber-400" : (nuvemErro || planilhaErro) ? "text-rose-400 fill-rose-400" : "text-emerald-400 fill-emerald-400"}`} />
                <span className="hidden sm:inline">{salvando ? "Salvando..." : nuvemErro ? "Sem conexão com a nuvem" : planilhaErro ? "Planilha não atualizada" : nuvemConfigurada() ? "Salvo na nuvem" : "Tudo salvo"}</span>
              </span>
...
function ModalPlanilhaOnline({ db, update, sincronizando, onSincronizar, onClose }) {
  ...
  const [webhook, setWebhook] = useState(db.planilhaWebhookUrl || "");
  const [enviando, setEnviando] = useState(false);
  const enviarAgora = async () => {
    const w = webhook.trim();
    if (!w.startsWith("https://script.google.com/")) return toast("Cole o link do Web App do Apps Script (script.google.com/macros/...).", "error");
    update((d) => { d.planilhaWebhookUrl = w; });
    setEnviando(true);
    try {
      await planilhaEnviar(w, db);
      toast("Dados enviados para a planilha (abas Cambistas, Lancamentos, Pagamentos e Gastos).", "success");
    } catch {
      toast("A planilha não respondeu. Confira se o Web App foi implantado com acesso \"Qualquer pessoa\".", "error");
    }
    setEnviando(false);
  };
  ...
  const salvarLink = () => {
    ...
    if (webhook.trim() && !webhook.trim().startsWith("https://script.google.com/")) return toast("Link do Web App inválido. Ele começa com script.google.com/macros/...", "error");
    update((d) => { d.planilhaUrl = url.trim(); d.planilhaWebhookUrl = webhook.trim(); });
    toast(url.trim() || webhook.trim() ? "Links da planilha salvos." : "Links removidos.", "success");
  };
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="border-t border-slate-100 pt-4">
            <div className="text-sm text-slate-600 mb-2">
              <span className="font-semibold">Enviar mudanças do site para a planilha</span> — configure o Web App do Apps Script (instruções no arquivo <span className="font-mono text-xs">COMO_ATIVAR_ESCRITA_PLANILHA.md</span>) e cole o link dele abaixo. Com isso, toda mudança feita aqui (lançamentos, pagamentos e <span className="font-semibold">gastos</span>) é espelhada na planilha automaticamente.
            </div>
            <label className={lbl}>Link do Web App (Apps Script)</label>
            <div className="flex gap-2">
              <input value={webhook} onChange={(e) => setWebhook(e.target.value)} className={`${inp} min-w-0`} placeholder="https://script.google.com/macros/s/.../exec" />
              <button onClick={enviarAgora} disabled={enviando}
                className="shrink-0 text-sm px-3 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-white font-medium transition-colors duration-150 disabled:opacity-50 inline-flex items-center gap-1.5">
                <RefreshCw size={13} className={enviando ? "animate-spin" : ""} /> {enviando ? "Enviando..." : "Enviar agora"}
              </button>
            </div>
          </div>
...
function ModalLancamento({ dados, onClose, onSave }) {
  ...
  return (
    <Modal titulo={`Novo Lançamento — ${dados.nome}`} onClose={onClose} onSave={salvar}>
      ...
      <div className="text-xs text-slate-400">Se o cambista <span className="font-medium text-rose-500">perdeu</span> na conta, toque no botão <span className="font-medium">+/−</span> ao lado do valor para deixá-lo negativo. Nesse caso o resultado vira saldo devedor (vermelho).</div>
    </Modal>
  );
}
